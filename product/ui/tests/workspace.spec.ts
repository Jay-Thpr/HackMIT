...
async function seekTo(page: Page, time: number) {
  const slider = page.getByRole('slider', { name: 'Incident timeline' })
...
test('judge guide introduces every workspace tab and returns after refresh', async ({ browser, baseURL }) => {
  ...
  const guide = page.getByRole('dialog', { name: 'What each workspace tab does' })
  await expect(guide).toBeVisible()
  const tabs = guide.getByRole('list', { name: 'Workspace tab guide' }).getByRole('listitem')
  await expect(tabs).toHaveCount(4)
  for (const name of ['Observability', 'Agent workspace', 'Incident replay', 'Evidence lineage']) await expect(guide.getByRole('heading', { name, exact: true })).toBeVisible()
  await page.screenshot({ path: 'test-results/judge-guide-desktop.png', fullPage: true })

  await guide.getByRole('button', { name: 'Open Observability', exact: true }).click()
  await expect(guide).not.toBeVisible()
  await expect(page.getByRole('button', { name: 'Observability', exact: true })).toHaveAttribute('aria-current', 'page')
  await page.reload()
  await expect(guide).toBeVisible()
  await page.getByRole('button', { name: 'Close dialog' }).click()
  await expect(guide).not.toBeVisible()

  await page.getByRole('button', { name: 'Judge guide', exact: true }).click()
  await expect(guide).toBeVisible()
  await page.setViewportSize({ width: 390, height: 844 })
  await expect(guide).toBeVisible()
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
  await page.screenshot({ path: 'test-results/judge-guide-mobile.png', fullPage: true })
  await page.getByRole('button', { name: 'Close dialog' }).click()
  await expect(guide).not.toBeVisible()
  await context.close()
})
...
for (const architecture of ['commerce', 'pipeline']) {
  test(`${architecture}: healthy start, explicit clone lifecycle, cleanup, and rewind`, async ({ page }) => {
    ...
    await page.getByRole('combobox', { name: 'System architecture' }).selectOption(architecture)
    ...
    await expect(page.getByRole('button', { name: 'Play incident', exact: true })).toBeVisible()
...
test('clone motion is seek-safe, paused, and archived clones stay with the winner emphasised', async ({ page }) => {
  ...
  await expect(cloneA).toHaveAttribute('data-presence', '1.000')
  await expect(page.getByRole('button', { name: 'Replay incident', exact: true })).toBeVisible()
})
...
test('the incident report opens by itself when playback reaches the end', async ({ page }) => {
  ...
  const report = page.locator('dialog.report-sheet[open]')
...
test('demo playback advances from healthy to clone startup and freezes when paused', async ({ page }) => {
  ...
  const cursor = await page.getByRole('slider', { name: 'Incident timeline' }).inputValue()
  await page.waitForTimeout(500)
  expect(await page.getByRole('slider', { name: 'Incident timeline' }).inputValue()).toBe(cursor)
  await expect(lifecycle).toHaveAttribute('data-phase', 'starting')
...
test('pages explain their jobs and offer meaningful empty-state actions', async ({ page }) => {
  ...
  await expect(page.getByText(/\d+ reference investigations · \d+ recorded incidents/)).toBeVisible()
...
test('incident replay includes the Commerce platform investigation and selects it across the workspace', async ({ page }) => {
  await openWorkspace(page)
  await page.getByRole('combobox', { name: 'System architecture', exact: true }).selectOption('pipeline')
  await page.getByRole('navigation', { name: 'Main navigation' }).getByRole('button', { name: 'Incident replay', exact: true }).click()
  const commerce = page.getByRole('listitem', { name: 'Reference investigation Commerce platform commerce', exact: true })
  await expect(commerce).toContainText('Commerce platform')
  await commerce.locator('.replay-list-main').click()
  await expect(page.getByRole('combobox', { name: 'System architecture', exact: true })).toHaveValue('commerce')
...
test('renders the real WebGL scene and a clearly marked, interactive prototype', async ({ page }) => {
  ...
  await openWorkspace(page)
  await seekTo(page, 47)
  await expect(page.getByText('Recorded dataset', { exact: true })).toBeVisible()
  await expect(page.locator('.map-canvas canvas')).toBeVisible()
  ...
  await expect(page.getByText('Instances: 0')).toBeVisible()
  await page.getByRole('tab', { name: /Decision trace/ }).click()
  await expect(page.getByText('Recorded investigation event').first()).toBeVisible()
...
test('context dropdowns use the workspace theme instead of default controls', async ({ page }) => {
  await openWorkspace(page)
  for (const name of ['System architecture', 'Selected environment']) {
    const select = page.getByRole('combobox', { name, exact: true })
    await expect(select).toHaveCSS('appearance', 'none')
    await expect(select).toHaveCSS('border-radius', '6px')
    await expect(select.locator('..')).toHaveCSS('background-color', 'rgb(43, 49, 45)')
    ...
    await select.focus()
    await expect(select.locator('..')).toHaveCSS('outline-color', 'rgb(183, 195, 150)')
  }
  await page.getByRole('combobox', { name: 'System architecture', exact: true }).selectOption('pipeline')
  await expect(page.getByRole('combobox', { name: 'System architecture', exact: true })).toHaveValue('pipeline')
...
test('seeking backward removes future environments and evidence', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Pause replay', exact: true }).last()).toBeVisible()
  await page.getByRole('button', { name: 'Pause replay', exact: true }).last().click()
  const before = await page.getByRole('slider', { name: 'Incident timeline' }).inputValue()
  await page.waitForTimeout(500)
  expect(await page.getByRole('slider', { name: 'Incident timeline' }).inputValue()).toBe(before)
})

test('supports a second arbitrary architecture, 3D view, metrics, and safety controls', async ({ page }) => {
  await openWorkspace(page)
  await seekTo(page, 47)
  await page.getByRole('combobox', { name: 'System architecture' }).selectOption('pipeline')
  await seekTo(page, 47)
  await expect(page.locator('.map-canvas canvas')).toBeVisible()
...
test('phase navigation synchronizes replay and previous event controls', async ({ page }) => {
  ...
  await expect(page.getByRole('slider', { name: 'Incident timeline' })).toHaveValue('72')
  ...
  await expect(page.getByRole('slider', { name: 'Incident timeline' })).toHaveValue('70')
...
test('spatial investigation and safety dialog meet core accessibility checks', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Safety & approvals', exact: true }).click()
...
test('loads a real incident from the Product API when requested', async ({ page }) => {
  ...
  await openWorkspace(page, `/?incident=${encodeURIComponent(incidents[0].id)}`)
  await expect(page.getByText('Live incident', { exact: true })).toBeVisible()
  await expect(page.locator('select[aria-label="System architecture"]')).toHaveValue(incidents[0].id)
  await expect(page.locator('.map-canvas canvas')).toBeVisible()
  await expect(page.getByText('Live incident · real audit log')).toBeVisible()
  await page.locator('select[aria-label="System architecture"]').selectOption('commerce')
  await expect(page.getByText('Recorded dataset', { exact: true })).toBeVisible()
})
...
for (const [diagnosis, confirmed, label] of [
  ...
  test(`diagnosis rendering follows ${diagnosis} / confirmed=${confirmed} and rewinds`, async ({ page }) => {
    ...
    await openWorkspace(page, `/?incident=${scenario.id}`)
    await expect(page.getByText('Live incident', { exact: true })).toBeVisible()
    await page.getByRole('slider', { name: 'Incident timeline' }).press('End')
    await page.getByRole('button', { name: 'Inspect primary-db in Production', exact: true }).click()
    await page.getByRole('tab', { name: 'Evidence', exact: true }).click()
    ...
    await page.getByRole('button', { name: 'Watch the agents in 3D' }).click()
    await page.getByRole('slider', { name: 'Incident timeline' }).press('ArrowRight')
