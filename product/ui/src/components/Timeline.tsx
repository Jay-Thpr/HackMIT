...
export function Timeline({ scenario }: { scenario: Scenario }) {
  ...
  return <div className="timeline">
    ...
    <div className="scrubber">
      <input aria-label="Simulation timeline" aria-valuetext={`${timeLabel(cursor)} of ${timeLabel(scenario.duration)}`} type="range" min={0} max={scenario.duration} step={1} value={cursor} onChange={event => seek(Number(event.target.value))} style={{ '--progress': `${cursor / scenario.duration * 100}%` } as React.CSSProperties} />
