...
export function Dialogs({ scenario, workspace }: { scenario: Scenario; workspace: WorkspaceState }) {
  ...
  useEffect(() => {
    if (dialog) { setSaved(false); element.current?.showModal() }
  ...
  return <dialog ref={element} className="workspace-dialog" onCancel={close} onClose={close} aria-labelledby="dialog-title" onClick={event => { if (event.target === element.current) { const bounds = element.current!.getBoundingClientRect(); if (event.clientX < bounds.left || event.clientX > bounds.right || event.clientY < bounds.top || event.clientY > bounds.bottom) close() } }}>
    ...
    {dialog === 'experiment' ? <form onSubmit={event => { event.preventDefault(); setSaved(true) }}>
      ...
      <label>Hypothesis<select required defaultValue={scenario.hypotheses[0].id}>{scenario.hypotheses.map(hypothesis => <option key={hypothesis.id} value={hypothesis.id}>{hypothesis.title}</option>)}</select></label>
      <div className="form-row"><label>Environment<select defaultValue="new"><option value="new">New isolated clone</option>{workspace.environments.filter(env => env.id !== 'production').map(env => <option key={env.id}>{env.label}</option>)}</select></label><label>Target<select defaultValue={scenario.targetId}>{scenario.topology.nodes.map(node => <option key={node.id} value={node.id}>{node.label}</option>)}</select></label></div>
      <label>Predicted response<textarea required placeholder="If this hypothesis is correct, I expect…" defaultValue={scenario.hypotheses[0].prediction} rows={3} /></label>
      ...
      <h2 id="dialog-title">Autonomy with boundaries.</h2><p className="dialog-subtitle">See exactly what is allowed, active, and waiting for a person.</p>
      <div className="safety-tier"><ShieldCheck size={17} /><div><strong>Automatic</strong><p>Telemetry reads and bounded, reversible interventions.</p></div></div>
