...
export function ExperimentLab({ scenario, workspace }: { scenario: Scenario; workspace: WorkspaceState }) {
  ...
  return <div className="lab-view">
    <section className="lab-banner"><div className="lab-banner-icon"><FlaskConical size={29} strokeWidth={1.2} /></div><div><span className="overline">A QUESTION BEFORE EVERY EXPERIMENT</span><h2>Test a theory. Not your luck.</h2><p>Reproduce a symptom in isolation, predict the response, then compare it with evidence.</p></div><button className="primary-button" onClick={() => set({ dialog: 'experiment' })}>Design an experiment <ArrowRight size={15} /></button></section>
    ...
    <div className="clone-card-grid">{scenario.hypotheses.map((hypothesis, index) => {
      const environmentId = index === 0 ? 'clone-a' : 'clone-b'
      const environment = workspace.environments.find(env => env.id === environmentId)
      return <article className="panel lab-clone-card" key={hypothesis.id}><div className="clone-card-top"><span className="clone-icon" style={{ color: hypothesis.color }}><GitBranch size={23} /></span><span className="quiet-badge">{environment ? 'Isolated · simulated' : 'Not active at this time'}</span></div><span className="overline">HYPOTHESIS {hypothesis.id}</span><h3>{hypothesis.title}</h3><p>{hypothesis.description}</p><dl><div><dt>Inherits</dt><dd>Topology, versions, workload</dd></div><div><dt>Does not inherit</dt><dd>Production data or hidden state</dd></div><div><dt>Observation</dt><dd>{environment ? `Through ${timeLabel(cursor)}` : 'No active clone'}</dd></div></dl><button className="secondary-button" onClick={() => { seek(index === 0 ? 44 : 47); set({ view: 'investigation', environmentId, selectedNode: undefined, focusRevision: useWorkspace.getState().focusRevision + 1 }) }}>Explore this reproduction <ArrowRight size={14} /></button></article>
    })}</div>
    <section className="panel lab-safety"><ShieldCheck size={23} /><div><h3>A clear boundary between exploration and execution.</h3><p>This prototype can preview designs, not create infrastructure. Real controls require a connected adapter, authentication, budgets, an explicit target, and confirmed rollback behavior.</p></div></section>
  </div>
}
...
export function ReplayLibrary({ scenario }: { scenario: Scenario }) {
  ...
  return <div className="replay-view">
    <section className="panel replay-hero"><span className="overline">INCIDENT MEMORY</span><h2>Every investigation leaves<br />a better test behind.</h2><p>A reproduction recipe captures the workload, intervention, and observations that recreated an incident. Future patches must survive the evidence, not just a unit test.</p><span className="quiet-badge">Design preview · no saved live incidents</span></section>
