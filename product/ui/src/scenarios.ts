...
function eventsFor(scenario: Omit<Scenario, 'events'>): WorkspaceEvent[] {
  ...
  const events: WorkspaceEvent[] = [
    make(0, 'baseline', 'Healthy reference captured', { actor: 'math', tool: 'telemetry.window', detail: 'A recorded healthy reference for this incident dataset.', result: 'Service-level metrics and topology inventory are available.', phase: 'Monitoring' }),
    ...
    make(44, 'observe', 'Both reproductions match the incident shape', { environmentId: 'clone-a', actor: 'math', targetId: target, tool: 'evidence.compare', detail: 'Compare the same metric window in production and the clone, with provenance and missing-data coverage.', prediction: 'Latency, issued load, and error rate should move together.', result: 'This run matches on all three displayed metrics. Reproduction alone does not confirm the cause.', causeId: `${id}-35-action` }),
    ...
    make(55, 'undo', 'Transient slowdown reverted', { environmentId: 'clone-a', actor: 'adapter', targetId: target, tool: 'lab.undo', undoId: 'perturb-a', detail: 'Reversion is explicitly confirmed in the recorded run rather than inferred from the countdown.' }),
    make(58, 'action', 'Test the lowest-impact separating intervention', { environmentId: 'clone-a', actor: 'investigator-a', targetId: policy, phase: 'Comparing interventions', tool: 'levers.apply', args: { max_retries: 0, ttl_s: 8 }, action: { id: 'cap-a', label: 'Retries capped · 0', ttl: 8 }, readings: { [target]: baseline[target], [policy]: baseline[policy], [entry]: baseline[entry] }, prediction: scenario.hypotheses[0].prediction, detail: 'A recorded retry-cap probe in Clone A. This is a diagnostic test, not a production action.' }),
    ...
    make(66, 'undo', 'Clone A stays healthy after release', { environmentId: 'clone-a', actor: 'adapter', targetId: policy, tool: 'levers.undo', undoId: 'cap-a', detail: 'The recorded metrics remain near their healthy reference after retries are restored.', result: 'A separating response, reproduced in a clone — not yet confirmed in production.' }),
    make(67, 'undo', 'Clone B does not recover after release', { environmentId: 'clone-b', actor: 'adapter', targetId: policy, tool: 'levers.undo', undoId: 'cap-b', readings: incident, detail: 'The recorded dependency remains constrained after retries are restored.', result: 'The two clone responses disagree. A short production probe can now discriminate.' }),
    make(72, 'action', 'Confirm with a reversible production probe', { actor: 'adapter', targetId: policy, phase: 'Confirming in production', tool: 'levers.apply', args: { max_retries: 0, ttl_s: 10 }, action: { id: 'cap-production', label: 'Retries capped · 0', ttl: 10 }, readings: { [target]: baseline[target], [policy]: baseline[policy], [entry]: baseline[entry] }, prediction: 'Recovery that persists after release supports a self-sustaining overload.', detail: 'One recorded production action with a TTL and registered undo.' }),
    ...
    make(96, 'verdict', 'Self-sustaining overload confirmed', { actor: 'math', diagnosis: 'A', confirmed: true, targetId: target, tool: 'judge.confirm', phase: 'Confirmed', detail: 'The production system stays healthy after the probe is released. The hypothesis passes its positive confirmation test.', result: 'Recorded outcome from the selected incident dataset.', causeId: `${id}-82-undo` }),
    ...
    make(108, 'archive', 'Clone B archived; evidence retained', { environmentId: 'clone-b', actor: 'adapter', phase: 'Investigation complete', tool: 'lab.destroy', detail: 'Both clones are removed. Production remains healthy, and all recorded evidence is available for review.' }),
  ]
  ...
  for (const [env, offset] of [['clone-a', 0], ['clone-b', 1]] as const) {
    ...
    const results = [
      ...
      { at: 69 + offset, checkId: 'release', expected: a ? 'Recovery persists after the cap is released.' : 'Overload returns after the cap is released.', observed: a ? 'Recovery persisted after release in the recorded window.' : 'Overload returned as predicted. This passing assertion does not mean the system is healthy.' },
    ]
    for (const result of results) events.push(make(result.at, 'observe', `${result.checkId} check passed`, { id: `${id}-${env}-${result.checkId}`, sequence: result.at + 100, environmentId: env, actor: 'math', tool: 'suite.evaluate', detail: 'Explicit test result from the selected incident dataset.', testResult: { ...result, passed: true }, result: result.observed }))
  }
...
// workers behind a transactional outbox). These recorded fixtures are not live
// telemetry, and the readings below remain bounded to the incident dataset.
...
/** The three recorded reference investigations built by `eventsFor`. They share one clone timeline:
