...
import { environmentLifecycleLabel, environmentOutcomeLabel, isObserverEnvironment, replay, timeLabel, visibleEvents, type IncidentLifecycle } from './model'
...
const viewTitles: Record<View, { eyebrow: string; title: string; subtitle: string }> = {
  ...
  replay: { eyebrow: 'THE EVIDENCE LIBRARY', title: 'Incident replay', subtitle: 'Choose a recorded investigation to watch again. Playback never reruns infrastructure actions.' },
...
const lifecycleSteps: { id: IncidentLifecycle; label: string; title: string; detail: string }[] = [
  { id: 'monitoring', label: 'Normal system', title: 'Healthy reference system', detail: 'Monitored services are at baseline. Play the investigation to follow an incident from its first symptoms to cleanup.' },
...
export default function App() {
  ...
  const playbackLabel = playing ? 'Pause replay' : workspace.lifecycle === 'complete' || cursor >= scenario.duration ? 'Replay incident' : cursor === 0 ? 'Play incident' : 'Resume replay'
  const toggleDemo = () => {
  ...
  return <div className={`app-shell ${view === 'investigation' ? 'is-spatial-workspace' : ''}`}>
    ...
    <aside className="sidebar">
      ...
      <div className="sidebar-bottom"><span className="preview-indicator" data-live={scenario.live || undefined}><i />RECORDED WORKSPACE</span><p>{scenario.live ? 'Replaying a recorded investigation.' : 'A safe space to investigate.'}</p><button className="sidebar-help" onClick={() => set({ dialog: 'guide' })}><CircleHelp size={15} />Judge guide<ArrowRight size={13} /></button><div className="profile"><span className="profile-avatar">FL</span><div><strong>Local workspace</strong><small>Recorded incident data</small></div><span className={scenario.live ? 'live-dot' : 'offline-dot'} /></div></div>
    </aside>

    <div className="main-shell">
      <header className="topbar"><div className="breadcrumb"><span>Workspace</span><ChevronRight size={12} /><strong>{navigation.find(item => item.view === view)?.label}</strong></div><div className="topbar-actions"><span className="demo-badge"><i />{scenario.live ? (scenario.complete ? 'Live incident' : 'Live incident · in progress') : 'Recorded dataset'}</span>{scenario.live && (scenario.source === 'api' ? <a className="pause-all" href={`/api/incidents/${encodeURIComponent(scenario.id)}/evidence.json`} download>Export evidence</a> : <button className="pause-all" onClick={() => exportEvidence(scenario)}>Export evidence</button>)}<span className="topbar-divider" /><button className="pause-all" onClick={() => set({ playing: false })} disabled={!playing}><Pause size={13} />Pause replay</button></div></header>
      <main id="main-content">
        ...
        <div className="context-bar"><div className="scenario-context"><label className="context-field"><span>System</span><span className="context-select"><Box size={14} /><select aria-label="System architecture" value={scenarioId} onChange={event => { setScenario(event.target.value); setEntitySearch('') }}>{scenarios.map(item => <option value={item.id} key={item.id}>{item.live ? item.incidentTitle : item.name}</option>)}</select></span></label><span className="context-type">{scenario.subtitle}</span></div><label className="context-field environment-context"><span>Environment</span><span className="context-select"><Layers3 size={14} /><select aria-label="Selected environment" value={environment.id} onChange={event => focus(event.target.value)}>{workspace.environments.map(env => <option key={env.id} value={env.id}>{env.label}</option>)}</select></span></label></div>

        <section className="workspace-facts" aria-label="Current investigation facts"><span><Waves size={13} />Signal <strong className={reading?.health === 'degraded' ? 'metric-warning' : ''}>{reading?.latency === undefined ? 'Not collected' : `${new Intl.NumberFormat('en-US').format(reading.latency)} ms`}</strong> <span className="fact-muted">at {targetName}</span></span><span><Sparkles size={13} />Question <strong>{questionLabel}</strong></span><span><GitBranch size={13} />Clones <strong>{workspace.environments.filter(env => env.id !== 'production' && !isObserverEnvironment(env)).length}</strong> <span className="fact-muted">isolated environments</span></span><span><ShieldCheck size={13} />Safety <strong>{workspace.actions.filter(action => action.environmentId === 'production').length} / 5</strong> actions <span className="fact-muted">{activeActions.some(action => action.environmentId === 'production') ? '· TTL bounded' : '· No active intervention'}</span></span></section>
        ...
        {view === 'investigation' ? <>
          <section className="lifecycle-summary" aria-label="Incident lifecycle" data-phase={workspace.lifecycle}>
            ...
            {(workspace.lifecycle === 'complete' || (scenario.complete && cursor >= scenario.duration)) && <div className="lifecycle-actions"><button className="text-button" onClick={() => set({ dialog: 'report' })}>Open the incident report <ArrowRight size={13} /></button><button className="text-button" onClick={() => set({ explanationOpen: true })}>Review the explanation <ArrowRight size={13} /></button></div>}
          </section>
          ...
          <div className={`workspace-grid ${showInspector ? 'has-inspector' : 'without-inspector'} ${expanded ? 'is-expanded' : ''}`}>
            <section className="panel topology-panel">
              ...
              <div className="map-canvas" data-testid="topology-stage" data-isolated-layer={isolatedLayer ?? 'all'}>
                ...
                <div className="map-status"><span className={`map-mode ${playing ? 'is-playing' : ''}`}><i />{playing ? 'REPLAY PLAYING' : cursor === 0 ? 'REPLAY READY' : cursor >= scenario.duration ? 'REPLAY COMPLETE' : 'REPLAY PAUSED'}</span><span>{scenario.live ? 'Readings are 5 s C1 windows; traffic shows aggregate request flow' : 'Traffic shows aggregate request flow'}</span></div>
        ...
        <footer className="page-footer"><span><Mark />Faultline <span>·</span> The model proposes. Measurement decides.</span><span>{scenario.live ? 'Real audit log · readings from Elasticsearch' : <>Recorded incident workspace <ArrowDownRight size={12} /></>}</span></footer>
      </main>
    </div>
