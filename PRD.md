...
**Where the LLM runs (v6.2 decision): OpenAI behind Elastic Cloud Agent Builder.** Every LLM job above is a *reasoning-over-context* job, and the context already lives in Elasticsearch: `faultline-fingerprints` (C1, production and per-clone), `faultline-audit` (C4: experiments, verdicts, actions), curated prior reports, reproduction recipes, similar past incidents, and the raw OTel traces. The Brain can sit behind Elastic Agent Builder as an OpenAI `chat_completion` inference endpoint (`faultline-openai-investigation`, see `faultline_brain.elastic_investigation`), reasoning through **closed, parameterized, read-only tools**: `faultline.incident_timeline`, `faultline.clone_vs_production`, `faultline.similar_incidents`, `faultline.incident_context`, and `faultline.semantic_incident_memory`. The fifth tool queries a Jina-backed `semantic_text` field and is operator context only, never verdict input. Rules that do not change:
...
**Current implementation and proof plan (2026-09-20).** The primary Elastic project is authoritative for C1 fingerprints and C4 audit events. A separate, asynchronous Observability mirror receives raw OTLP and a durable display copy of C1/C4; it never becomes a second source of truth. Owner 2 has implemented explicit index templates, Query DSL stores and analytics, a bounded ES|QL incident timeline, five fixed Agent Builder tools, and curated incident memory using Elastic's managed `.jina-embeddings-v3` `semantic_text` endpoint. Tool definitions and the Agent Builder client are covered by local tests, but a live deployment, managed-OTLP ingestion, Jina smoke retrieval, Agent Builder conversation, and Kibana evidence view must be verified before the demo.

**Sponsor gap to close.** Add one narrowly scoped Elastic Workflow that assembles a read-only incident evidence packet from C1, C4, clone comparison and semantic memory after an incident reaches report stage. The workflow may prepare human review; it must not choose a cause, issue a lever action, or bypass the noise-model judge. This makes the Elastic Workflow usage visible without weakening Faultline's safety boundary.
